...
export const SosheIQLogo: React.FC<SosheIQLogoProps> = ({
  ...
  const stubStemBaseLeftX = iconWidth * 0.40;
  const stubStemBaseRightX = stubStemBaseLeftX + stubStemBaseWidth;
  
  const stemPointY = iconHeight + 3;
  const stemMidX = (stubStemBaseLeftX + stubStemBaseRightX) / 2 + (iconWidth * 0.08);
  ...
  const bubblePath = `
    ...
    A ${cornerRadius},${cornerRadius} 0 0 1 ${iconWidth - cornerRadius},${iconHeight}
    L ${stubStemBaseRightX},${iconHeight}
    Q ${stemMidX + (stubStemBaseRightX - stemMidX)*0.6},${iconHeight + 1.5} ${stemMidX},${stemPointY}
    Q ${stemMidX - (stemMidX - stubStemBaseLeftX)*0.6},${iconHeight + 1.5} ${stubStemBaseLeftX},${iconHeight}
  ...
  const smilePath = `M ${smileStartX},${smileY} Q ${iconWidth / 2},${smileControlY} ${smileEndX},${smileY}`;

  const originalFontSize = 14;
  const textFontSize = originalFontSize * 1.15 * 1.10; // Approx 17.71
  ...
  return (
    ...
    >
      <g>
        ...
        <path d={smilePath} stroke={smileFill} strokeWidth="2" fill="none" strokeLinecap="round" />
      </g>
      ...
      >
        <tspan fill={textFill}>Sosh</tspan>
        <tspan fill={textFill}>e</tspan>
